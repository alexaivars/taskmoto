...
//   return store;
// }
var endpoint = typeof window === "undefined" ? "https://localhost:3000/graphql" : "/graphql";
function createApolloClient(initialState, ctx) {
    // The `ctx` (NextPageContext) will only be present on the server.
    // use it to extract auth headers (ctx.req) or similar.
    var enchancedFetch = function (url, init) {
        return fetch(url, __assign(__assign({}, init), { headers: __assign(__assign({}, init.headers), { Cookie: ctx.req.headers.cookie }) })).then(function (response) { return response; });
    };
